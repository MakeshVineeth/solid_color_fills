...
class ScaffoldHome extends StatefulWidget {
  ...
  @override
  ScaffoldHomeState createState() => ScaffoldHomeState();
}

class ScaffoldHomeState extends State<ScaffoldHome> with RestorationMixin {
  ...
  final widgetsList = <Widget>[HomeTab(), CustomColorPicker(), FeedScreen()];
    ...
    if (Platform.isAndroid) {
      quickActions.initialize((shortcutType) {
        ...
        if (shortcutType == AppShortcuts.collectionQuickAction.type) {
          ...
          index = 1;
        } else if (shortcutType == AppShortcuts.feedQuickAction.type) {
          index = 2;
        }
  ...
  void _onItemTapped(int index) {
    if (mounted && index < widgetsList.length) {
      ...
      _bottomAppBarKey.currentState?.animateTo(
        index,
      ); // ConvexAppBar doesn't navigate by itself.
      setTabIndex(index);
    }
  }
